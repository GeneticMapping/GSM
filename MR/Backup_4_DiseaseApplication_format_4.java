...
public class DiseaseApplication_format_4 {
    ...
    public static class Map extends Mapper<LongWritable, Text, Text, Text> {
	  ...
	  private HashMap<String, ArrayList<String>> GenesIndex = null;
	  private String currentGene = null;
	  private ArrayList<String> currentRefSeq = null;
	  ...
	  public void map(LongWritable key, Text value, Context context)
		...
		while (tokenizer.hasMoreTokens()) {
			...
			if(!refSeq.equals(patSeq)){ //&& !patSeq.contains("n") && !patSeq.contains("y") && !patSeq.contains("r") && !patSeq.contains("m") && !patSeq.contains("w") && !patSeq.contains("k") && !patSeq.contains("s")) {
				context.write(new Text(IDs[0]), new Text(IDs[1]+"_"+codonID+"_"+refSeq+"_"+patSeq));
			}
    ...
    public static class Reduce extends Reducer<Text, Text, Text, Text> {
	  private Path[] localFiles;	  
	  private HashMap<String, HashMap<Integer, String>> EnsemblDB = null;
	  private ArrayList<String> currentGeneDB = null;
	  
	  @Override
	  public void setup(Context context) throws IOException, InterruptedException {
		EnsemblDB = new HashMap<String, HashMap<Integer, String>>();
		HashMap<Integer, String> AuxRefSeq = new HashMap<Integer, String>();
		...
		for(int i=0; i<localFiles.length; i++) {
			...
			if(localFiles[i].toString().indexOf("db/") != -1){
				...
				while ((line = br.readLine()) != null) {
					AuxRefSeq.put(new Integer(line.split("\\s+")[0]), line);
				}
				...
				//AuxRefSeq.remove(AuxRefSeq.size() - 1);
				EnsemblDB.put(geneName, AuxRefSeq);
				
				AuxRefSeq = new HashMap<Integer, String>();
			}
		}
	  }
      ...
      public void reduce(Text key, Iterable<Text> values, Context context)
        ...
        Text value = null;
		Integer codon_number = null;
		HashMap<Integer, String> EnsemblDBHash = EnsemblDB.get(key.toString());
		
		Iterator<Text> valuesList = values.iterator();
		
        while (valuesList.hasNext()) {
          value = valuesList.next();
		  codon_number = new Integer(value.toString().split("_")[1]);
		  
		  if(EnsemblDBHash.get(codon_number) != null){
			//String reduceOutput = value.toString().concat(" - DB data: " + EnsemblDBHash.get(codon_number));
			context.write(key, new Text(value.toString() + " - DB data: " + EnsemblDBHash.get(codon_number)));
		  }
		  
		  //currentGeneDB = EnsemblDB.get(key.toString());
		  
		  /*for(int i=0; i<currentGeneDB.size(); i++){
			if(currentGeneDB.get(i).split(" ")[0].equals(codon_number)){
				String reduceOutput = value.toString().concat(" - DB data: " + currentGeneDB.get(i));
				context.write(key, new Text("- " + reduceOutput));
				break;
			}
		  }*/
        }
